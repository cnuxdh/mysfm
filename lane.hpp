...
public:
    ...
    LaneDetect(Mat startFrame)
    {
        ...
        int rows = startFrame.rows;
        int cols = startFrame.cols;

        currFrame = Mat(rows, cols, CV_8UC1, 0.0);                        //initialised the image size to 320x480
        ...
        vanishingPt = 0; //currFrame.rows / 2;                           //for simplicity right now
        ROIrows = currFrame.rows - vanishingPt;               //rows in region of interest
        minSize = 2;        // 0.00015 * (currFrame.cols*currFrame.rows);  //min size of any region to be selected as lane
        maxLaneWidth = 128; //0.025 * currFrame.cols;                     //approximate max lane width based on image size
        smallLaneArea = 7 * minSize;
        longLane = 0.8 * currFrame.rows;
        ratio = 4;

        //these mark the possible ROI for vertical lane segments and to filter vehicle glare
        vertical_left  = 0;// 2 * currFrame.cols / 5;
        vertical_right = 0;// 3 * currFrame.cols / 5;
        vertical_top   = 0;// 2 * currFrame.rows / 3;
    ...
    void blobRemoval()
    {
        ...
        imwrite("c:\\temp\\midstep.jpg", temp);
        imwrite("c:\\temp\\laneBlobs.jpg", temp2);
        imwrite("c:\\temp\\lane.jpg", currFrame);
    }
...
DLL_EXPORT int DetectLane(Mat& image );

DLL_EXPORT int CVDetectLane(string filepath);
